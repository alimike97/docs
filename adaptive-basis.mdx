...
When the adaptive ROM model is used (`solver_mode = AROM`), the reduced basis is updated dynamically. Several options for basis adaptation are available in ROMify that can be selected via parameter \``arom_method`\` in input file.
